...
public class IdpCreateUserIfUniqueAuthenticator extends org.keycloak.authentication.authenticators.broker.IdpCreateUserIfUniqueAuthenticator {
	...
	protected void authenticateImpl(final AuthenticationFlowContext context, final SerializedBrokeredIdentityContext serializedCtx, final BrokeredIdentityContext brokerContext) {
		...
		final ExistingUserInfo duplication = brokerContext.getIdpConfig().isTransientUsers() ? null : checkExistingUser(context, username, serializedCtx, brokerContext);

		UserModel federatedUser = null;
		if (brokerContext.getIdpConfig().isTransientUsers()) {
			logger.debugf("Transient brokering requested. Recording user details for account '%s' and from identity provider '%s' .", username, brokerContext.getIdpConfig().getAlias());
